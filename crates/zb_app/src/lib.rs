...
use zb_domain::tweaks::definitions::{
    DisableGameDvrTweak, DisableTransparencyTweak, SetHighPerformanceTweak,
    ShowFileExtensionsTweak,
};
use zb_infrastructure::logging::init_logging;
use zb_infrastructure::persistence::audit_logger::InMemoryAuditLogger;
use zb_infrastructure::persistence::sqlite_repo::InMemorySnapshotRepo;
...
pub fn run() {
    ...
    let tweaks: Vec<Arc<dyn zb_domain::tweaks::Tweak>> = vec![
        Arc::new(DisableGameDvrTweak),
        Arc::new(DisableTransparencyTweak),
        Arc::new(SetHighPerformanceTweak),
        Arc::new(ShowFileExtensionsTweak),
    ];

    let snapshot_service = InMemorySnapshotRepo::new();
    let audit_service = InMemoryAuditLogger::new();
