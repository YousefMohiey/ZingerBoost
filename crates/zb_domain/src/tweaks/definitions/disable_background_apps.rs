...
impl Tweak for DisableBackgroundAppsTweak {
    ...
    async fn apply(&self) -> Result<TweakResult, TweakError> {
        if let Some(provider) = &self.provider {
            ...
            provider
                .write(&path, "GlobalUserDisabled", &RegValue::Dword(1))
                .await
                .map_err(|e| TweakError::Registry(e.to_string()))?;
        }
        Ok(TweakResult {
            reboot_required: false,
            message: "Background apps disabled. UWP apps will no longer run in the background."
                .into(),
        })
    }
    ...
    fn explain(&self) -> TweakExplanation {
        TweakExplanation {
            ...
            why_it_helps:
                "Reduces CPU, RAM, and battery usage by stopping background UWP app processes."
                    .into(),
            potential_risks: Some(
                "Live tiles will not update, and some apps may not receive push notifications."
                    .into(),
            ),
