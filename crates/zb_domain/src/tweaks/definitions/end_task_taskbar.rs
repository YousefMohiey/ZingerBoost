...
impl Tweak for EndTaskOnTaskbarTweak {
    fn metadata(&self) -> TweakMetadata {
        TweakMetadata {
            ...
            description: "Adds an 'End Task' option when right-clicking a program in the taskbar.".into(),
            ...
            source_url: Some("https://winutil.christitus.com/dev/tweaks/essential-tweaks/endtaskontaskbar".into()),
        }
    }

    async fn is_applied(&self) -> Result<bool, TweakError> {
        if let Some(provider) = &self.provider {
            let path = RegPath::hkcu(r"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced\TaskbarDeveloperSettings");
    ...
    async fn capture_state(&self) -> Result<SnapshotData, TweakError> {
        if let Some(provider) = &self.provider {
            let path = RegPath::hkcu(r"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced\TaskbarDeveloperSettings");
            let val = provider.read(&path, "TaskbarEndTask").await.unwrap_or(RegValue::Dword(0));
            Ok(SnapshotData::Registry { path, name: "TaskbarEndTask".into(), previous: val })
        } else {
            Ok(SnapshotData::Registry {
                path: RegPath::hkcu(r"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced\TaskbarDeveloperSettings"),
    ...
    async fn apply(&self) -> Result<TweakResult, TweakError> {
        ...
        Ok(TweakResult { reboot_required: false, message: "End Task enabled on taskbar.".into() })
    }
    ...
    async fn revert(&self, snapshot: &SnapshotData) -> Result<TweakResult, TweakError> {
        ...
        Ok(TweakResult { reboot_required: false, message: "End Task restored.".into() })
    }

    fn explain(&self) -> TweakExplanation {
        TweakExplanation {
            what_it_does: "Enables the 'End Task' option in the right-click context menu of taskbar items.".into(),
            why_it_helps: "Quickly kill frozen or unresponsive programs without opening Task Manager.".into(),
